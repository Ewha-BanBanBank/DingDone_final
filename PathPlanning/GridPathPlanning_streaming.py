...
while (n < flight):
    notLast = True
    # 이렇게 구현하면 드론이 한 방향으로 이동한 후의 한 장면만 보여주게됨.
    # 주행 동안의 모든 비디오를 streaming하려면 sleep없이 while True로 매시간 frame_read받아서 imshow해줘야함.
    # 따라서 while문을 더 촘촘히 짧은 간격으로 많이 돌리던지, 아니면 while True와 while (n<flight) 각각 따로 multiprocessing으로 구현해서
    # 다른 함수를 동시에 실행가능하게 하던지 해야함.
    img_ = me.get_frame_read().frame
    img_ = cv2.resize(img_, (360, 240))
    cv2.imshow("Map", img_)
    cv2.waitKey(1)
