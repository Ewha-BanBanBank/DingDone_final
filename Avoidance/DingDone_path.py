...
def Drone(send_img, send_yaw, recv_rc, recv_ack): # control drone and streaming video
    ...
    grid = 5  # (cm)
    row = 28  # (*0.5 grid) # larger than 1
    col = 1  # (grid) # larger than 1
    x, y = 0, 0
    c = 0
    r = 0
    speed = grid # 1초에 grid(cm)만큼씩 이동
    lat = 5 #latency of depth estimation
    start, end = 0, 0
    ...
    while True:
        ...
        while True:
            ...
            img = cv2.resize(img, (64 * 4, 48 * 4)) # do not change image size
            ...
            speed, y, avoid , sleep_ = recv_rc.recv()
...
def compDepth(send_avoid, send_ack, recv_img):
    ...
    while True:
        ...
        #if first_lat>0: sleep(first_lat); first_lat = 0
        ack = True
